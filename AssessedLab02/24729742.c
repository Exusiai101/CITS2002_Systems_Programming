...
struct Process_Node* CreateProcess_Node() {
  ...
  struct Process_Node* newNode =
      (struct Process_Node*)malloc(sizeof(struct Process_Node));
  if (newNode == NULL) {
...
struct Process_Node* Process_append(struct Process_Node* current,
...
struct Process_Node* readfile(const char* FileName, struct Process_Node* node) {
  ...
  int* PrsNo = (int*)calloc(1, sizeof(int));
    char* nameBuffer = (char*)calloc(1000, sizeof(char));
    int* quantaBuffer = (int*)calloc(10, sizeof(int));
    int* stepBuffer = (int*)calloc(10, sizeof(int));
  ...
  while (fgets(LineBuffer, 1000, InputFile)) {
    ...
    int itemsRead = sscanf(
        ...
        nameBuffer, &pi->TotalExecutionTime, &quantaBuffer[0], &stepBuffer[0],
        &quantaBuffer[1], &stepBuffer[1], &quantaBuffer[2], &stepBuffer[2],
        &quantaBuffer[3], &stepBuffer[3], &quantaBuffer[4], &stepBuffer[4],
        &quantaBuffer[5], &stepBuffer[5], &quantaBuffer[6], &stepBuffer[6],
        &quantaBuffer[7], &stepBuffer[7], &quantaBuffer[8], &stepBuffer[8],
        &quantaBuffer[9], &stepBuffer[9]);
    pi->name = calloc(1, sizeof(char) * strlen(nameBuffer) + 1);
    strcpy(pi->name, nameBuffer);

    if (itemsRead >= 2) {
      // Name, Total ExecutionTime read successful
    ...
    // clean up the cache prepare for reuse
    memset(nameBuffer, 0, sizeof(char) * 1000);
    memset(quantaBuffer, 0, sizeof(int) * 10);
    memset(stepBuffer, 0, sizeof(int) * 10);
  }
...
bool CompletionCheck(struct process* Process) {
  ...
  if (Process->CurrentRunningTime >= Process->TotalExecutionTime &&
...
void ProcessComplete(struct process *Process) {
  // Update Process info and global info
  Process->Completed = true;
  (*ProcessCompleted) ++ ;
}

void ProcessUpdateWithIOCall(struct process *Process) {
  // Update process info and global info when IO call happens
  int TimeToAdd = Process -> IOCallStep[Process -> IOCallMade];
  Process -> CurrentRunningTime += TimeToAdd;
  (*TotalRunningTime) += TimeToAdd;
}

void ProcessUpdateWithoutIOCall(struct process *Process) {
  // Update process info and global info when no IO call

  Process ->CurrentRunningTime ++;
  (*TotalRunningTime) ++;
}
  ...
  while (*ProcessCompleted < *TotalProcess) {
    if (current->data->Completed != true) {
      ...
      if (IOCallCheck(current->data)) {
        ProcessUpdateWithIOCall(current->data);
        if (CompletionCheck(current->data)) {
          ProcessComplete(current->data);
          // printf("ProcessCompleted was incremented to
          // %d\n",*ProcessCompleted);
        }

        // printf("%s made an IO call at %d\n", current ->data->name, current ->
        // data -> IOCallQuanta[current -> data -> IOCallMade]);
        current->data->IOCallMade++;
      }

      // if no io call
      else {
        // Completion Check every 1 ms
        for (int i = 0; i < TIME_QUANTUM; i++) {
          ProcessUpdateWithoutIOCall(current->data);
          if (CompletionCheck(current->data)) {
            ProcessComplete(current->data);
            // printf("ProcessCompleted was incremented to
            // %d\n",*ProcessCompleted);
...
int main(int argc, char* argv[]) {
  ...
  if (TotalRunningTime == NULL || TotalProcess == NULL ||
  ...
  if (readfile(FileName, node0) == NULL) {
