...
struct process
{
    ...
    int IoCallQuanta[MAXIMUM_IO_CALLS]; // Array of time quanta when io call occurs
...
{   
    ...
    printf("IO Quanta: ");
    for (int i = 0; i < MAXIMUM_IO_CALLS; i++) {
...
void ProcessSimu(struct process Process[])
{
    int i = 0;
...
{   
    ...
    struct process pr[MAX_PROCESS_NUM] = {0};
    struct process * p = pr;
